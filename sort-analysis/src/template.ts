...
export class Template {
    ...
    private attachActions(store: any): void {
        document
    ...
    private render(store: any): void {
        const appState: AppState = store.getState();

        // print graph based on Array
        ...
        // show list of algorithm results
        ...
        if (appState.data.simulation == null) {
            ...
            document.querySelector('#simulation_name').innerHTML = 'Simulation';
        } else {
            this.buttons.forEach((button: string) => (this.button(button).disabled = false));
            const simulation = appState.data.simulation;
            const steps = formatNumber(appState.data.step);
            const total = formatNumber(simulation.steps.length - 1);

            document.querySelector('#steps').innerHTML = `${steps} of ${total}`;
